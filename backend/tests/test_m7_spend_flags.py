...
async def _fixture(session):
    ...
    session.add(user)
    await session.flush()
    chk = AccountLogical(household_id=hh.id, owner_user_id=user.id, label="Checking", type="checking", currency="USD", is_shared=False)
    card = AccountLogical(household_id=hh.id, owner_user_id=user.id, label="Card", type="credit", currency="USD", is_shared=False)
