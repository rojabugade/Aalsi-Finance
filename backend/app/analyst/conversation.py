...
async def get_or_create_thread(session: AsyncSession, user: User, key: str) -> AnalystThread:
    ...
    if row is None:
        await session.execute(
            insert(AnalystThread)
            .values(household_id=user.household_id, key=key)
            .on_conflict_do_nothing(constraint="uq_analyst_thread_household_key")
        )
...
async def append_turn(
    ...
    *,
    question: str,
    answer: str,
    analyst_payload: dict | None = None,
) -> None:
